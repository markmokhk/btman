...
def run_mt4_report(terminal_folder, start_ini_path, setfile_path):
...
def run_report_thread(start_ini_path, setfile_path):

    random_folder = copy_to_random_folder(mother_folder)
    result = run_mt4_report(random_folder, start_ini_path, setfile_path)
...
def run_multiple_reports(start_ini_paths, setfile_path):
    ...
    def thread_callback():
        nonlocal active_threads, completed_threads
        active_threads -= 1
        completed_threads += 1
        print(f"Thread completed. {completed_threads}/{total_threads} finished.")
        start_next_thread()

    def start_next_thread():
        nonlocal active_threads
        if start_ini_paths:
            start_ini_path = start_ini_paths.pop(0)
            thread = threading.Thread(target=run_report_thread, args=(start_ini_path, setfile_path))
            thread.start()
            threads.append(thread)
            active_threads += 1
            print(f"Thread {completed_threads + 1} started")
            thread.join()
            thread_callback()

    # Start off with 8 concurrent threads
    for _ in range(min(max_concurrent_threads, len(start_ini_paths))):
        start_next_thread()
...
# Example usage:
#start_ini_paths = [r"c:\btman\start.ini"]
#run_multiple_reports(start_ini_paths, os.path.join(PROG_FOLDER, "TestExpertParameters.set"))

bt(symbol="AUDNZD", expert="Moving Average", setfile="TestExpertParameters.set", period="H1", from_date=datetime(2024, 7, 1), to_date=datetime(2024, 7, 31), every_period="3D")
