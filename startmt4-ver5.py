...
def run_mt4_report(terminal_folder, start_ini_path, setfile_path, thread_number=0):
    ...
    #process = subprocess.Popen([terminal_exe_path, start_ini_path, "/portable"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    process = subprocess.Popen([terminal_exe_path, start_ini_path, "/portable"],     stdout=subprocess.PIPE,     stderr=subprocess.PIPE, creationflags=subprocess.CREATE_NO_WINDOW) 
...
def run_multiple_reports(start_ini_paths, setfile_path):
    ...
    def run_thread(start_ini_path, thread_number):
        run_report_thread(start_ini_path, setfile_path, thread_number)
        print(f"Thread {thread_number} is completed.")

    with ThreadPoolExecutor(max_workers=8) as executor:
        futures = []
        for thread_number, start_ini_path in enumerate(start_ini_paths):
            futures.append(executor.submit(run_thread, start_ini_path, thread_number))
...
def bt(symbol, expert, setfile, period, from_date, to_date, every_period):
    # period: The timeframe for the backtest (e.g., 'M1', 'M5', 'M15', 'M30', 'H1', 'H4', 'D1', 'W1', 'MN
...
#bt(symbol="AUDNZD", expert="Moving Average", setfile="TestExpertParameters.set", period="H1", from_date=datetime(2024, 7, 15), to_date=datetime(2024, 7, 31), every_period="1D")
